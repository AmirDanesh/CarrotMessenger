...
public static class ServiceBuilderExtensions
{
    ...
    {
        builder.Services.AddDbContext<CarrotMessengerIdentityDbContext>(options =>
            options.UseInMemoryDatabase("CarrotMessengerDb"));
    ...
    public static void AddApplicationIdentity(this WebApplicationBuilder builder)
    {
        var identitySettings = builder.Configuration.GetSection("identity") ??
                               throw new ArgumentNullException("identitySettings");
        builder.Services.Configure<IdentitySettings>(identitySettings.Bind);
        var identityOptions = identitySettings.Get<IdentitySettings>() ??
    ...
    {
        app.MapPost("/identity/signin", async ([FromBody] SignInRequest signInRequest,
                IHttpContextAccessor httpContextAccessor, IOptions<IdentitySettings> identityOptions,
        ...
        app.MapPost("/identity/signup",
                async ([FromBody] SignUpRequest request, UserManager<ApplicationUser> userManager) =>
                {
    ...
    private static string CreateToken(IdentitySettings identityOptions, ApplicationUser user,
